...
public class PGS_아이템줍기 {
    ...
    public static void main(String[] args) {
        ...
        sol(characterX * 2, characterY * 2, itemX * 2, itemY * 2, new boolean[101][101]);
    ...
    public static void draw(int X1, int Y1, int X2, int Y2) {
        for (int i = Y1 ; i <= Y2 ; i++) {
            for (int j = X1 ; j <= X2 ; j++) {
                if (map[i][j] == 1 && (i == Y1 || i == Y2 || j == X1 || j == X2)) {
                    continue;
                }
                // 1.1. 테두리이면 2를 넣는다.
    ...
    public static void sol(int characterX, int characterY, int itemX, int itemY, boolean[][] visited) {
